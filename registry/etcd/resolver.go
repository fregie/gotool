...
func NewResolver(endpoints []string, service string) resolver.Builder {
	return &Resolver{endpoints: endpoints, service: "/service/" + service}
}
...
func (r *Resolver) watch(prefix string) {
	...
	if err == nil {
		for i, kv := range resp.Kvs {
			...
			addrDict[string(resp.Kvs[i].Value)] = resolver.Address{Addr: net.JoinHostPort(info.IP, strconv.Itoa(info.Port))}
		}
	}
